...
function init() {
  ...
  window.addEventListener('unload', () => sampleRUM('unload'));

  window.addEventListener('click', (event) => {
    const target = event.target.closest('A');
    if (target) sampleRUM('click', { target: target.href });
  });
}

/**
 * Waits for the LCP candidate element to be ready.
 * @param {Array} lcpBlocks Array of block names that are LCP candidates
 */
